...
__device__ int minVal(int a, int b) {
  return a > b ? b : a;
}
                   ...
                   const float* const filter, const int filterWidth)
{
  ...
  for (int filter_r = -filterWidth / 2; filter_r <= filterWidth / 2; ++filter_r) {
    for (int filter_c = -filterWidth / 2; filter_c <= filterWidth / 2; ++filter_c) {
      int image_r = minVal(maxVal(y + filter_r, 0), static_cast<int>(numRows - 1));
      int image_c = minVal(maxVal(x + filter_c, 0), static_cast<int>(numCols - 1));
      float image_value = static_cast<float>(inputChannel[image_r * numCols + image_c]);
      float filter_value = filter[(filter_r + filterWidth / 2) * filterWidth + filter_c + filterWidth / 2];
      result += image_value * filter_value;
    }
  }
  outputChannel[x + y * numCols] = result;
}
                          ...
                          const float* const filter, const int filterWidth)
{
  ...
  if (threadIdx.x < filterWidth && threadIdx.y < filterWidth) {
    shared_filter[filterWidth * threadIdx.y + threadIdx.x] = filter[filterWidth * threadIdx.y + threadIdx.x];
  }
  __syncthreads();
                      ...
                      unsigned char* const blueChannel)
{
  ...
  uchar4 cur = inputImageRGBA[x + y * numCols];
  redChannel[x + y * numCols] = cur.x;
  greenChannel[x + y * numCols] = cur.y;
  blueChannel[x + y * numCols] = cur.z;
}
                        ...
                        const int filterWidth)
{
  ...
  /* separateChannels<<<gridSize, blockSize, sizeof(uchar4) * numRows * numCols>>>(d_inputImageRGBA, numRows, numCols, d_red, d_green, d_blue); */
  ...
  int shared_size = sizeof(float) * filterWidth * filterWidth;
